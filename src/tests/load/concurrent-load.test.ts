...
afterEach(() => {
    vi.useRealTimers()
    // Flush queue entries immediately so the 5-second grace-period
    // setTimeout in enqueueForUser never leaks into subsequent test suites.
    __drainQueues()
})
...
describe('Test 10 — Full day simulation: time-bucketed stimulus + mode transitions', () => {
    ...
    it('Time bucket 1 (8am) — commute query fires weather+commute stimulus', () => {
        ...
        const commuteStimulus: StimulusInput = {
            type: 'weather',
        ...
        const score = computeFusionScore(commuteStimulus, userCtx)
        const decision = fusionProactiveDecision(commuteStimulus, userCtx)
