...
describe('Test 6 — Recovery protocol (#149): 3 positive interactions re-enable PROACTIVE', () => {
    ...
    it('after recovery: fusionProactiveDecision uses softer threshold (0.85)', () => {
        ...
        // Recovery is triggered — decision should not be DROP from pushback alone
        // (BACK_OFF is triggered by recentPushbacks, which takes priority over recovery)
        // checkRecovery only applies if pushback action is ALLOW — revisit
        // With recentPushbacks=2, evaluatePushback returns BACK_OFF, which returns DROP
        // This is correct behavior: pushback protocol wins
